...
_FAST_PATTERNS = [
    ...
    (re.compile(r'(?:elimina|borra|quita|suprime|eliminar|borrar|quitar|suprimir)\s+(?:todas\s+|todos\s+|todo\s+)?(?:mis\s+|las\s+)?(?:metas\s+de\s+ahorro|metas\s+de\s+ahorros|metas|ahorros|ahorro)\s*$', re.IGNORECASE),
     lambda m: {"intencion": "eliminar", "eliminar_objeto": "meta_ahorro", "eliminar_todas": True, "confianza": 0.98}),
...
- Para "eliminar": si el usuario quiere borrar un PRESUPUESTO (ej: "elimina el presupuesto de comida"), usa eliminar_objeto: "presupuesto" y categoria: "comida". Para transacciones usa eliminar_objeto: "transaccion". Para una META DE AHORRO (ej: "elimina la meta de ahorro del regalo de mi novia", "borra mi meta del carro"), usa eliminar_objeto: "meta_ahorro" y categoria: la etiqueta de la meta (ej: "regalo de mi novia"). Si el usuario quiere eliminar TODAS sus metas de ahorro ("elimina todas mis metas", "borra todos mis ahorros"), usa eliminar_objeto: "meta_ahorro" y eliminar_todas: true. NUNCA uses "presupuesto" ni "transaccion" para metas de ahorro.
...
def _validar_resultado(datos: dict) -> dict:
    ...
    eliminar_todas = datos.get("eliminar_todas")
    if eliminar_todas is True:
        resultado["eliminar_todas"] = True
