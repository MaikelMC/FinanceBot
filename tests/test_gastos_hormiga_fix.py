...
_contador = [0]
_base_uid = int(time.time())  # Evita colisión con datos de corridas previas en el DB compartido.


def _crear_usuario():
    _contador[0] += 1
    return database.obtener_o_crear_usuario(_base_uid + _contador[0], "FixHormiga")
...
class TestCategoriaInterpretada(unittest.TestCase):
    def setUp(self):
        database.crear_tablas()
        self.usuario = _crear_usuario()
        self.mon = database.crear_moneda(self.usuario["id"], "Peso", "", "CUP", es_default=True)
    ...
    def test_recarga_telefono_no_es_comida(self):
        # La transacción quedó etiquetada como "comida" (error de la transacción),
        # pero la descripción dice recarga -> debe reportarse como "recarga".
        _sembrar_gasto(
            self.usuario, self.mon, 360.0,
            "me recargue el teléfono y gasté 360 cup de saldo", "comida",
        )
        gastos = database.obtener_gastos_hormiga(self.usuario["id"], dias=30)
        self.assertEqual(len(gastos), 1)
        self.assertEqual(gastos[0]["categoria"], "recarga")

    def test_pizza_va_a_comida_rapida(self):
        _sembrar_gasto(
            self.usuario, self.mon, 400.0,
            "luego me compré una pizza de 400 cup", "almuerzo",
        )
        gastos = database.obtener_gastos_hormiga(self.usuario["id"], dias=30)
        self.assertEqual(gastos[0]["categoria"], "comida rápida")

    def test_cafe_va_a_cafe(self):
        _sembrar_gasto(
            self.usuario, self.mon, 120.0,
            "café con leche en la esquina", "comida",
        )
        gastos = database.obtener_gastos_hormiga(self.usuario["id"], dias=30)
        self.assertEqual(gastos[0]["categoria"], "café")

    def test_sin_palabra_clave_usa_categoria_almacenada(self):
        # "grabaciones" no coincide con ninguna palabra clave -> conserva la guardada.
        _sembrar_gasto(
            self.usuario, self.mon, 50.0,
            "hice un gasto de 50 cup en grabaciones", "Refresco",
        )
        gastos = database.obtener_gastos_hormiga(self.usuario["id"], dias=30)
        self.assertEqual(gastos[0]["categoria"], "Refresco")

    def test_nota_registra_categoria_interpretada(self):
        # La detección debe pasar: la categoría de la transacción está en activas.
        database.guardar_config_gastos_hormiga(self.usuario["id"], {
            "umbral_base": 5000.0, "umbral_moneda": "CUP", "frecuencia_minima": 3,
            "categorias_auto": "comida,recarga",
        })
        t = database.agregar_transaccion(
            self.usuario["id"], 0, "gasto", 360.0,
            "me recargue el teléfono 360 cup", moneda_id=self.mon["id"],
        )
        knowledge._nota_gasto_hormiga(
            self.usuario, 360.0, "me recargue el teléfono 360 cup", "comida", self.mon, t["id"]
        )
        gastos = database.obtener_gastos_hormiga(self.usuario["id"], dias=30)
        self.assertEqual(len(gastos), 1)
        self.assertEqual(gastos[0]["categoria"], "recarga")

    def test_interpretar_categoria_hormiga_unit(self):
        f = knowledge._interpretar_categoria_hormiga
        self.assertEqual(f("me recargue el teléfono 360 cup", "comida"), "recarga")
        self.assertEqual(f("pizza de 400", "almuerzo"), "comida rápida")
        self.assertEqual(f("taxi a la oficina", "otros"), "transporte")
        self.assertEqual(f("texto sin coincidencia", "Refresco"), "Refresco")
        self.assertEqual(f("", None), "otros")
